...
void PlayerAi::update(Actor* owner) {
	...
	if (dx != 0 || dy != 0) {
		...
		if (engine.map->isHeal(owner->x + 1, owner->y + 1) || engine.map->isHeal(owner->x - 1, owner->y - 1)) {
			engine.player->destructible->heal(1);
		}
	}
}
...
void EnemyAi::update(Actor *owner) {
	...
	if (engine.player->getDistance(owner->x, owner->y) < 25) {
